...
- Remove the Railway `Procfile` and all current Railway deployment/setup guidance. Update `README.md`, `docs/ARCH_TBEP.md`, configuration comments, and repository diagrams so they describe the Cloudflare Worker request lifecycle and no longer claim that the app binds a port or registers its webhook on startup.
